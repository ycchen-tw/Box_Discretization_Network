...
_indexes = sorted([f.split('.')[0].split('_')[1]
                   for f in os.listdir(os.path.join(root_path, 'bdn_label_file'))])

if phase == 'train':
  indexes = [line for line in _indexes if int(line) >= split]  # only for this file
  # indexes = [line for line in _indexes]
elif phase == 'train_bal':
  indexes = [line for line in _indexes if int(line)%split != 0]
elif phase == 'valid_bal':
  indexes = [line for line in _indexes if int(line)%split == 0]
else:
...
for index in tqdm(indexes):
  ...
  im = cv2.imread(os.path.join(root_path, 'img/') + 'img_' + index + '.jpg')
  ...
  anno_file = os.path.join(root_path, 'bdn_label_file/') + 'img_' + index + '.txt'
